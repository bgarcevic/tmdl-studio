...
namespace TmdlStudio.Services
{
    ...
    public class FabricApiService
    {
        ...
        /// <summary>
        /// Gets item display name by item id.
        /// </summary>
        private static async Task<string> GetItemDisplayNameAsync(string workspaceId, string itemId, string accessToken)
        {
            try
            {
                var items = await ListItemsByTypeAsync(workspaceId, "SemanticModel", accessToken);
                var match = items.FirstOrDefault(item => string.Equals(item.Id, itemId, StringComparison.OrdinalIgnoreCase));
        ...
        /// <summary>
        /// Gets an item by logical ID and type.
        /// Returns both id and current display name for optional rename operations.
        /// </summary>
        private static async Task<(string id, string displayName)> GetItemByLogicalIdAsync(
            string workspaceId,
            string logicalId,
            string itemType,
            string accessToken,
            params string[] preferredNames)
        {
            try
            {
                var candidates = await ListItemsByTypeAsync(workspaceId, itemType, accessToken);
                ...
                var orderedCandidates = candidates
                    .OrderBy(candidate =>
                    {
                        ...
                        var index = preferred.FindIndex(name => string.Equals(name, displayName, StringComparison.OrdinalIgnoreCase));
                ...
                foreach (var candidate in orderedCandidates)
                {
                    ...
                    string itemLogicalId = await GetItemLogicalIdAsync(workspaceId, id, accessToken);
                    if (string.Equals(itemLogicalId, logicalId, StringComparison.OrdinalIgnoreCase))
                    {
                        return (id, candidate.DisplayName);
                    }
                }
        ...
        /// <summary>
        /// Gets logicalId from an item's .platform definition.
        /// </summary>
        private static async Task<string> GetItemLogicalIdAsync(string workspaceId, string itemId, string accessToken)
        {
            try
            {
                using var httpClient = new HttpClient();
                string url = $"{FabricApiBase}/workspaces/{workspaceId}/items/{itemId}/getDefinition";

                using var request = new HttpRequestMessage(HttpMethod.Post, url);
                request.Headers.Authorization = new AuthenticationHeaderValue("Bearer", accessToken);

                var response = await httpClient.SendAsync(request);
                var definitionContent = await ReadOperationResultBodyAsync(httpClient, response, accessToken);
                if (string.IsNullOrWhiteSpace(definitionContent))
                {
                    return null;
                }

                using var document = JsonDocument.Parse(definitionContent);
                if (!document.RootElement.TryGetProperty("definition", out var definitionElement) ||
                    !definitionElement.TryGetProperty("parts", out var partsElement))
                {
                    return null;
                }
                ...
                {
                    string path = part.TryGetProperty("path", out var pathElement) ? pathElement.GetString() : null;
                    if (!string.Equals(path, ".platform", StringComparison.OrdinalIgnoreCase))
                    {
                        continue;
                    }

                    string payload = part.TryGetProperty("payload", out var payloadElement) ? payloadElement.GetString() : null;
                    ...
                    try
                    {
                        platformJson = Encoding.UTF8.GetString(Convert.FromBase64String(payload));
                    }
                    catch
                    {
                        continue;
                    ...
                    using var platformDoc = JsonDocument.Parse(platformJson);
                    if (!platformDoc.RootElement.TryGetProperty("config", out var configElement) ||
                        !configElement.TryGetProperty("logicalId", out var logicalIdElement))
                    {
                        continue;
                    }

                    var logicalId = logicalIdElement.GetString();
                    return string.IsNullOrWhiteSpace(logicalId) ? null : logicalId;
                }
